...
static void remove_save(int test)
{
	...
	snprintf(home, sizeof(home), "%s/.config/zelda-3t", getenv("HOME"));
	mkdir(home, 0755);
	snprintf(directory, sizeof(directory), "%s/zelda3t-%d.dat", home, test);
	remove(directory);
}
...
void Keyboard::loadOldP() {
    ...
    ifstream f2(directory,ios::in | ios::binary);
    if(f2.is_open()) {
        int i;
