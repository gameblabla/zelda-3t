...
Generique::~Generique() {
    ...
    if (imageFond1) SDL_FreeSurface(imageFond1);
...
void Generique::initTroc() {
    ...
    gpJeu->affiche(image, "SAC DE TROC", 38, 16);
    ...
    switch (i) {
        case 0 :
            ...
            && gpJeu->getKeyboard()->getRang(i+6)) 
                gpJeu->affiche(image, "RANG : Héros Légendaire", 72, 104);
            ...
            || (gpJeu->getKeyboard()->getRang(i+3) && gpJeu->getKeyboard()->getRang(i+6))) 
                gpJeu->affiche(image, "RANG : Héros des deux Mondes", 72, 104);
            else if (gpJeu->getKeyboard()->getRang(i)) 
                gpJeu->affiche(image, "RANG : Héros d'Hyrule", 72, 104);
            else if (gpJeu->getKeyboard()->getRang(i+3)) 
                gpJeu->affiche(image, "RANG : Héros de Termina", 72, 104);
            else if (gpJeu->getKeyboard()->getRang(i+6)) 
                gpJeu->affiche(image, "RANG : Héros du Temps", 72, 104);
            else gpJeu->affiche(image, "RANG : INCONNU", 72, 104);
            gpJeu->affiche(image, "Finir le jeu à 100%.", 72, 120);
            break;
        case 1 :
            cadre(64,80,192,80);
            int l; l=88;
